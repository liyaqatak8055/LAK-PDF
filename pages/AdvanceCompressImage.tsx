...
export const AdvanceCompressImage: React.FC = () => {
  ...
  const handleDownloadAll = async () => {
    ...
    if (results.length === 1) {
      const img = results[0];
      downloadFile(img.compressedBlob, `compressed-${targetKB}kb-${img.file.name.split('.')[0]}.jpg`);
    } else {
      const zip = new JSZip();
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-slate-900 mb-4">Advance Image Compress</h1>
