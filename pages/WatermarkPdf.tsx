...
import { Type, X, Image as ImageIcon, Check, Grid3X3, RotateCw, LayoutGrid } from 'lucide-react';
...
export const WatermarkPdf: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      downloadPdf(watermarkedBytes, `watermarked-${file.name}`);
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-1/3 space-y-6">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm">
                    ...
                    <button onClick={() => setFile(null)} className="text-slate-400 hover:text-red-500"><X size={20} /></button>
                </div>
                ...
                <div className="relative w-full max-w-md aspect-[1/1.41] bg-white shadow-xl flex overflow-hidden">
                    ...
                    <div className="absolute inset-0 p-8 pointer-events-none -z-10 opacity-20 flex flex-col gap-4">
                        {Array.from({length: 20}).map((_,i) => (
                            <div key={i} className="h-2 bg-slate-800 rounded w-full" style={{ width: `${Math.random() * 40 + 60}%` }} />
                        ))}
                    </div>
                </div>
            </div>
