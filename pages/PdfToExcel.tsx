...
import { convertPdfToExcel } from '../services/officeService';
...
export const PdfToExcel: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      downloadFile(blob, `${file.name.replace('.pdf', '')}.xlsx`);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-slate-900 mb-4">PDF to Excel</h1>
        <p className="text-lg text-slate-500 max-w-2xl mx-auto">
          Convert PDF data into editable Excel spreadsheets (XLSX).
        </p>
      </div>
