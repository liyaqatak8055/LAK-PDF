...
export const ComparePdf: React.FC = () => {
  ...
  useEffect(() => {
    ...
    renderPages();
  }, [pdf1Doc, pdf2Doc, currentPage, scale, viewMode]);
  ...
  const renderPages = async () => {
    ...
    try {
      ...
      const pageNum1 = Math.min(currentPage, pdf1Doc.numPages);
      const pageNum2 = Math.min(currentPage, pdf2Doc.numPages);

      const page1 = await pdf1Doc.getPage(pageNum1);
      const page2 = await pdf2Doc.getPage(pageNum2);
          ...
          <div ref={containerRef} className="flex flex-col items-center justify-center min-h-[600px]">
            ...
            `}>
              ...
              <div className={`
                 ...
                 ${viewMode === 'overlay' ? 'opacity-50 mix-blend-multiply bg-transparent border-none shadow-none pointer-events-none' : ''}
              `}>
                 {viewMode === 'side-by-side' && <div className="bg-slate-100 p-2 text-xs font-bold text-center text-slate-500 border-b">Modified</div>}
                 <canvas ref={canvas2Ref} className="block mx-auto" />
              </div>
